...
const { SecureChatContract } = require('..');
...
describe('SecureChatContract', () => {
    ...
    beforeEach(() => {
        contract = new SecureChatContract();
        ctx = new TestContext();
    ...
    describe('#secureChatExists', () => {

        it('should return true for a private asset that exists', async () => {
            await contract.secureChatExists(ctx, '001').should.eventually.be.true;
        });

        it('should return false for a private asset that does not exist', async () => {
            await contract.secureChatExists(ctx, '002').should.eventually.be.false;
        });

    });

    describe('#createSecureChat', () => {

        it('should throw an error for a private asset that already exists', async () => {
            await contract.createSecureChat(ctx, '001').should.be.rejectedWith('The asset secure chat 001 already exists');
        });
        ...
        it('should throw an error if transient data is not provided when creating private asset', async () => {
            ...
            await contract.createSecureChat(ctx, '002').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should create a private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.createSecureChat(ctx, '002');
            ctx.stub.putPrivateData.should.have.been.calledOnceWithExactly(collectionName, '002', Buffer.from('{"privateValue":"1500"}'));
        });

    });

    describe('#readSecureChat', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.readSecureChat(ctx, '003').should.be.rejectedWith('The asset secure chat 003 does not exist');
        });

        it('should return my private asset', async () => {
            await contract.readSecureChat(ctx, '001').should.eventually.deep.equal({ privateValue: '150' });
            ctx.stub.getPrivateData.should.have.been.calledWithExactly(collectionName, '001');
        });

    });

    describe('#updateSecureChat', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.updateSecureChat(ctx, '003').should.be.rejectedWith('The asset secure chat 003 does not exist');
        });

        it('should throw an error if transient data is not provided when updating private asset', async () => {
            let transientMap = new Map();
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateSecureChat(ctx, '001').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should update my private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateSecureChat(ctx, '001');
    ...
    describe('#deleteSecureChat', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.deleteSecureChat(ctx, '003').should.be.rejectedWith('The asset secure chat 003 does not exist');
        });

        it('should delete my private asset', async () => {
            await contract.deleteSecureChat(ctx, '001');
            ctx.stub.deletePrivateData.should.have.been.calledOnceWithExactly(collectionName, '001');
        });

    });

    describe('#verifySecureChat', () => {
        ...
        it('should return success message if hash provided matches the hash of the private data', async () => {
            ...
            ctx.stub.getPrivateDataHash.withArgs(collectionName, '001').resolves(Buffer.from(hashToVerify, 'hex'));
            const result = await contract.verifySecureChat(ctx, mspid, '001', '{"privateValue":"125"}');
        ...
        it('should throw an error if hash provided does not match the hash of the private data', async () => {
            ...
            const result = await contract.verifySecureChat(ctx, mspid, '001', 'someObject');
        ...
        it('should throw an error when user tries to verify an asset that doesnt exist', async () => {
            ...
            await contract.verifySecureChat(ctx, mspid, '001', 'someObject').should.be.rejectedWith('No private data hash with the key: 001');
        });
    });
});
